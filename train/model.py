...
class Decoder(torch.nn.Module):
    ...
    def forward(self, Q_convs,M_convs, memory, H, W):
        ...
        return pred, D1_D2_D3


class MMN_VSOD(nn.Module):
    def __init__(self):
        super(MMN_VSOD, self).__init__()
        ...
        self.motion = nn.Sequential(
            nn.Conv2d(64*3*2, 64*2, 3, padding=1, bias=False), nn.BatchNorm2d(64*2), nn.ReLU(),
    ...
    def forward(self, x):
        ...
        for query_t in range(N):
            ...
            pred, motion_feature= self.decoder(Q_convs, M_convs, memory, H=input_H, W=input_W)
            if query_t in [1,2]:
        ...
        if len(motion_features)>0:
            motion = torch.sigmoid(self.motion(torch.cat(motion_features,dim=1)))
            return preds, motion
        return preds
